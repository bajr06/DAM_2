...
public class GeneradorProceso {
	public void ejecutar(String ruta, String [] args) {
		...
		// Esto hace que el proceso herede la entrada salida estandar del proceso padre. Así podemos ver el resulado del comando (ipconfig en este caso).
		
		pb.inheritIO(); // Heredar la entrada y salida del padre.

		try {
			Process proceso = pb.start(); //es lo mismo que pb.start();
			// Ahora toca obtener el codigo de retorno para saber si se ha ejecutado bien el programa y para ello esperamos a que se acabe el proceso, por eso debemos de esperar a que acabe con:
